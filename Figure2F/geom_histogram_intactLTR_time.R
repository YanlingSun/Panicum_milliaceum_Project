...
data <- read.delim("intact.time2", header=T, sep="\t")
#type	value
#PmiA	0.00
data$type=factor(data$type, levels=c("PmiB","PmiA"))

p1 <- ggplot(data, aes(x=time, fill=type)) + 
  geom_histogram(position="identity", binwidth=0.2, alpha=0.8) + 
  ...
  xlab("Insertion time of intact LTRs (Mya)") + 
  ylab("Count") 

p1

ggsave(p1, filename="intact.time2.pdf", width=5, height=2)
